...
class Solution:
    def openLock(self, deadends, target):
        ...
        while len(combos) > 0:
            ...
            currentCombo = combos.popleft()
            currentBreadth -= 1

            if currentCombo == target:   
                return steps
            elif currentCombo in deadends or currentCombo in usedCombos:
                continue
            else:
                usedCombos.add(currentCombo)
                combos.extend(self.comboBFS(currentCombo))

        return -1
    def comboBFS(self, combo):
        newCombos = []
        for i, char in enumerate(combo):
            num = int(char)
            newCombos.append(combo[:i] + str((num + 1) % 10) +  combo[i+1:])
            newCombos.append(combo[:i] + str((num - 1) % 10) +  combo[i+1:])
        return newCombos
